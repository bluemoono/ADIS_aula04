...
public class ManterFilmesController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(inserir) {
		case "mostrar":
			String filme = request.getParameter("titulo"+"descricao"+"diretor"+"genero"+"populariedade"+"data_lancamento"+"poster");
			int filme = Integer.parseInt(id_filme);
			FilmeService filmeService = new FilmeService();
			Filme filme = filmeService.inserirFilme(filme);
			System.out.println(filme);
			
			request.setAttribute("filme", filme);
			
			RequestDispatcher view = request.getRequestDispatcher("InserirFilme.jsp");
			view.forward(request, response);
			break;
		case "inserir":
			break;
